...
# Resampling Methods: evaluating models by Caret
...
# ROSE
set.seed(9560)
rose_train <- ROSE(y ~ ., data  = itrain)$data                           
table(rose_train$y)
...
# GLM
set.seed(2019)
fit.glm <- train(y~., data=rose_train, method="glm", family=binomial(link='logit'), metric=metric, trControl=control)
print(fit.glm)
plot(varImp(fit.glm),15, main = 'GLM feature selection')

# RANDOM FOREST
set.seed(2019)
fit.rf <- train(y~., data=rose_train, method="rf", metric=metric, trControl=control)
print(fit.rf)
plot(varImp(fit.rf),15, main='Random Forest feature selection')

# NNET
set.seed(2019)
fit.nnet <- train(y~., data=rose_train, method="nnet", metric=metric, trControl=control)
print(fit.nnet)
plot(varImp(fit.nnet),15, main = 'Neural Network feature selection')

# GBM
set.seed(2019)
fit.gbm <- train(y~., data=rose_train, method="gbm", metric=metric, trControl=control, verbose=F)
print(fit.gbm)
par(mar = c(4, 11, 1, 1))
summary(fit.gbm, cBars=15, las=2, plotit=T, main = 'GBM feature selection')
